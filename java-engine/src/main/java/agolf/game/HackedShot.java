...
public class HackedShot implements Runnable {
    ...
    public void run() {
        ...
        */
        ...
        if (this.strokeInterrupted) {
            ...
            this.resultOnHole = new boolean[this.playerCount];
            for (int i = 0; i < this.playerCount; ++i) {
                this.resultOnHole[i] = this.onHoleSync[i].get();
            }
    ...
    public boolean[] getResultOnHole() {
        if (this.resultOnHole == null) {
            this.resultOnHole = new boolean[this.playerCount];
            for (int i = 0; i < this.playerCount; ++i) {
                this.resultOnHole[i] = this.onHoleSync[i].get();
            }
        }
        return this.resultOnHole;
    }

    public long getSeedRaw() {
        return this.seed.getRaw();
    }
    ...
    private void recordFrame() {
        double[] frame = new double[this.playerCount * 2];
        for (int i = 0; i < this.playerCount; ++i) {
            frame[i * 2] = this.playerX[i];
            frame[i * 2 + 1] = this.playerY[i];
        }
